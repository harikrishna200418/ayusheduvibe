document.addEventListener('DOMContentLoaded', () => {
    ...
    /* =========================================
       NEW: CHECK LOGIN STATUS & UPDATE UI
       ========================================= */
    ...
    if (isLoggedIn === 'true') {
        ...
        const registerBtn = navLinksContainer ? navLinksContainer.querySelector('a[href="register.html"]') : null;

        const profileHtml = `
            <div style="display: flex; align-items: center; gap: 10px; margin-left: 10px;">
                <div style="width: 35px; height: 35px; background: var(--primary); color: #000; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold;">
                    ${userName.charAt(0).toUpperCase()}
                </div>
                <span style="color: var(--text-main); font-weight: 600;">Hi, ${userName.split(' ')[0]}</span>
            </div>
        `;

        if (registerBtn) {
            registerBtn.outerHTML = profileHtml;
        } else if (navLinksContainer) {
            // If button not found (other pages), just append
            ...
            if (uni.scholarship) {
                scholarshipHTML = `
                    <div style="margin: 1rem 0; padding: 1rem; background: ${uni.scholarship.color}15; border-radius: 8px; border-left: 3px solid ${uni.scholarship.color};">
                        ...
                        <p style="color: #fff; font-weight: bold; font-size: 0.9rem;">${uni.scholarship.desc}</p>
                    </div>
                `;
            }
    ...
    function renderJobs(data) {
        ...
        data.forEach(job => {
            ...
            card.innerHTML = `
                ...
                <button class="btn btn-secondary btn-full mt-2" onclick="applyForJob('${job.title}')">Apply Now</button>
            `;
    ...
    if (accomContainer && typeof APP_DATA !== 'undefined') {
        APP_DATA.accommodation.forEach(item => {
            ...
            card.className = 'card';
            card.innerHTML = `
                <div style="height: 150px; background: #2a3b55; margin-bottom: 1rem; border-radius: 8px; display:flex; align-items:center; justify-content:center; color: #a0aec0; font-size: 2rem;">
                    🏠
                </div>
                <h3>${item.title}</h3>
                <p>${item.location}</p>
